...
static void btif_avk_handle_event(UINT16 event, char* p_param)
{
    ...
    switch (event)
    {
        ...
        case BTIF_AVK_SINK_CONFIG_REQ_EVT:
            ...
            break;
        case BTA_AVK_A2DP_ALREADY_CONNECTED:
            BTIF_TRACE_ERROR("BTA_AVK_A2DP_ALREADY_CONNECTED");
            index = -1;
            memcpy(&bt_addr1, &(p_bta_data->rc_open.peer_addr), sizeof(bt_bdaddr_t));

            if (bt_avk_callbacks != NULL) {
              HAL_CBACK(bt_avk_callbacks, connection_state_cb, BTAV_CONNECTION_STATE_ALREADY_CONNECTION, &bt_addr1);
            }
            break;
        default:
