    ...
    if(event == AVDT_CONNECT_IND_EVT)
    {
        ...
        if(!p_lcb)
        {
            ...
            if (xx == BTA_AVK_NUM_LINKS)
            {
                ...
                AVDT_DisconnectReq (p_data->str_msg.bd_addr, NULL);
		  memcpy(rc_open.peer_addr,p_data->str_msg.bd_addr,6);
                if(p_cb->p_cback!=NULL){
                  (*p_cb->p_cback)(BTA_AVK_A2DP_ALREADY_CONNECTED, (tBTA_AVK *) &rc_open);
                }
                return;
            }
        }
...
tBTA_AVK_FEAT bta_avk_sink_check_peer_features (UINT16 service_uuid)
{
    ...
    if(service_uuid==UUID_SERVCLASS_AV_REM_CTRL_TARGET){
      fp = fopen(AVRC_TARGET_VERSION_CONF_FILE, "wb");
	  sprintf(ver, "0x%x", peer_rc_version);
	  ver[strlen(ver)]='\0';
