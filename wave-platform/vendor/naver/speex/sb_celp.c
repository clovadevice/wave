...
void *sb_encoder_init(const SpeexMode *m)
{
   ...
   for (i=0;i<st->lpcSize;i++){
      ...
      st->old_lsp[i]= DIV32(MULT16_16(25736, i+1), st->lpcSize+1);
   }
      ...
      if (!SUBMODE(innovation_quant)) {/* 1 for spectral folding excitation, 0 for stochastic */
         ...
         if (SUBMODE(double_codebook)) {
            ...
            for (i=0;i<st->subframeSize;i++)
               ...
               target[i]=MULT16_16_P13(20480, target[i]);

            SUBMODE(innovation_quant)(target, st->interp_qlpc, bw_lpc1, bw_lpc2, 
                                      SUBMODE(innovation_params), st->lpcSize, st->subframeSize, 
                                      innov2, syn_resp, bits, stack, st->complexity, 0);
            //signal_mul(innov2, innov2, MULT16_32_P15(QCONST16(0.4f,15),scale), st->subframeSize);
            signal_mul(innov2, innov2, MULT16_32_P15(13107,scale), st->subframeSize);
         ...
         if (st->innov_rms_save)
         {
            ...
            //st->innov_rms_save[sub] = MULT16_16_Q15(23171, compute_rms(innov, st->subframeSize));
         }
