...
void btsock_rfc_cleanup_spp(void) {
  pthread_mutex_lock(&slot_lock);
  for (size_t i = 0; i < ARRAY_SIZE(rfc_slots); ++i) {
    if (rfc_slots[i].id)
      cleanup_rfc_slot(&rfc_slots[i]);
...
static rfc_slot_t *find_rfc_slot_by_addr(bt_bdaddr_t bd_addr) {
  for (size_t i = 0; i < ARRAY_SIZE(rfc_slots); ++i){
  	if(memcmp(&rfc_slots[i].addr, &bd_addr, sizeof(bt_bdaddr_t)) == 0)
      return &rfc_slots[i];
  	}
...
static void cleanup_rfc_slot_spp(rfc_slot_t *slot) {
...
static uint32_t on_srv_rfc_connect(tBTA_JV_RFCOMM_SRV_OPEN *p_open, uint32_t id) {
  ...
  bdaddr_to_string(&accept_rs->addr, str, 18);

  spp_send_device_connect_status(str, 1);
...
static void on_rfc_close(UNUSED_ATTR tBTA_JV_RFCOMM_CLOSE *p_close, uint32_t id) {
  ...
  bdaddr_to_string(&slot->addr, str, 18);
  spp_send_device_connect_status(str, 0);
...
int bta_co_rfc_data_incoming(void *user_data, BT_HDR *p_buf) {
  ...
  spp_send_rcv_data(str,p_buf->data+p_buf->offset,p_buf->len);
...
string_to_bdaddr(addr, &bd_addr);
slot = find_rfc_slot_by_addr(bd_addr);

if(slot != NULL){
  ret = PORT_WriteData (slot->rfc_port_handle, data, len, &length);
}
