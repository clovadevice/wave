...
static int set_callbacks(spp_callbacks_t *callbacks);
static int btsock_send_data(char *addr, char *data, int len);
static int btsock_disconnect();

static int thread_handle = -1;
static thread_t *thread;
static spp_callbacks_t *bt_spp_cbacks = NULL;
...
static int set_callbacks(spp_callbacks_t *callbacks){
  ...
  bt_spp_cbacks = callbacks;
return 0;
}

void spp_send_rcv_data(char *addr, char *data, int len) {

if(bt_spp_cbacks){
            HAL_CBACK(bt_spp_cbacks,spp_rcv_data_cb, addr, data, len);
}

return;
}

void spp_send_device_connect_status(char *addr, int onoff) {

if(bt_spp_cbacks){
            HAL_CBACK(bt_spp_cbacks,spp_dev_connnect_cb, addr, onoff);
}
...
static int btsock_send_data(char *addr, char *data, int len)
{
  return  send_data_test(addr, data, len);
}
