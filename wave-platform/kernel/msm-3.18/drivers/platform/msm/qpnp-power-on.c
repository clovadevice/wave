...
qpnp_config_reset(struct qpnp_pon *pon, struct qpnp_pon_config *cfg)
{
	...
	rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_RESIN_S2_CNTL2(pon),
				QPNP_PON_S2_CNTL_EN, 0);
	if (rc) {
		dev_err(&pon->spmi->dev, "Unable to configure pwr key and reset in S2 disable\n");
		return rc;
	}

	/*
	gpio_direction_output(GPIO_RESIN,1);
	usleep_range(100, 120);
	gpio_direction_output(GPIO_RESIN,0);
*/
	/* enable S2 reset */
	rc = qpnp_pon_masked_write(pon, cfg->s2_cntl2_addr,
				QPNP_PON_S2_CNTL_EN, QPNP_PON_S2_CNTL_EN);
