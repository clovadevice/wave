				 ...
				 const char *buf, size_t len)
{
	...
	gpio_set_value(LED_TRIG1, 0);
	udelay(200);
	gpio_set_value(LED_TRIG1, 1);
	mutex_unlock(&chip->lock);
			...
			const struct i2c_device_id *id)
{
	...
	gpio = LED_TRIG1;
	ret = gpio_request(gpio, "LED_TRIG1");
	if (ret < 0) {
		printk(KERN_ERR "Failed to request GPIO%d "
            "ret=%d\n",gpio, ret);
    	} else {
	...
	/*
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 3, 0x32);
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 4, 0x82);
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 5, 0xC8);
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 6, 0x32);
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 7, 0x82);
	lp55xx_write(chip, LP5523_REG_LED_PWM_BASE + 8, 0xC8);
	*/
