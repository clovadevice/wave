				 ...
				 const char *buf, size_t len)
{
	...
	gpio_set_value(LED_TRIG2, 0);
	udelay(200);
	gpio_set_value(LED_TRIG2, 1);
	mutex_unlock(&chip->lock);
...
static LP55XX_DEV_ATTR_RW(gpio_value, show_gpio_value, store_gpio_value);
...
void lp552308_animation(int index, int mode)
{
	int i;
	...
	for (i = 0; i < 9; i++)
		lp55xx_write(global_chip, LP5523_REG_LED_PWM_BASE + i, color_pattern[mode][(index % 12) * 9 + i]);
	mutex_unlock(&global_chip->lock);
}
EXPORT_SYMBOL(lp552308_animation);
			...
			const struct i2c_device_id *id)
{
	...
	gpio = LED_TRIG2;
	ret = gpio_request(gpio, "LED_TRIG2");
	if (ret < 0) {
		printk(KERN_ERR "Failed to request GPIO%d "
            "ret=%d\n",gpio, ret);
    	} else {
