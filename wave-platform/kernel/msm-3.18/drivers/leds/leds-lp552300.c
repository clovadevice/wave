				 ...
				 const char *buf, size_t len)
{
	...
	gpio_set_value(LED_TRIG0, 0);
	udelay(200);
	gpio_set_value(LED_TRIG0, 1);
	mutex_unlock(&chip->lock);
			...
			const struct i2c_device_id *id)
{
	...
	gpio = LED_TRIG0;
	ret = gpio_request(gpio, "LED_TRIG0");
	if (ret < 0) {
		printk(KERN_ERR "Failed to request GPIO%d "
            "ret=%d\n",gpio, ret);
    	} else {
