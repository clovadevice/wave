...
void lp552310_animation(int index, int mode)
{
	int i;
	...
	if (index < 6) {
		for ( i = 0; i < 9; i++)
			lp55xx_write(global_chip, LP5523_REG_LED_PWM_BASE + i, color_pattern[mode][11* 9 + i]);
	} else if (index > 6) {
		for (i = 0; i < 9; i++)
			lp55xx_write(global_chip, LP5523_REG_LED_PWM_BASE + i, color_pattern[mode][((index-6) % 12) * 9 + i]);
	}
	mutex_unlock(&global_chip->lock);
}
EXPORT_SYMBOL(lp552310_animation);
