...
static ssize_t stk_als_lux_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	als_lux = als_lux * cali_gain / 10000;
	mutex_unlock(&ps_data->io_lock);
...
static ssize_t stk_als_adc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct stk3x1x_data *ps_data = dev_get_drvdata(dev);
	int32_t als_reading;
	mutex_lock(&ps_data->io_lock);
...
static ssize_t stk_als_gain_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", cali_gain);
}

static ssize_t stk_als_gain_store(struct device *dev,
			struct device_attribute *attr, char *buf, size_t len)
{
	kstrtoul(buf, 10, &cali_gain);
	return len;
}
...
static struct device_attribute als_adc_attribute = __ATTR(adc,0444,stk_als_adc_show,NULL);
...
static struct device_attribute als_cali_gain_attribute =
	__ATTR(calibration_gain, 0664, stk_als_gain_show,
	stk_als_gain_store);
...
module_i2c_driver(stk_ps_driver);
