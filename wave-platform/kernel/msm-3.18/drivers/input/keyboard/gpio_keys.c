				...
				const struct gpio_keys_button *button)
{
	...
	if (error < 0) {
       ...
       error = gpio_request_one(UEI_IR_RESET,GPIOF_DIR_OUT,"ir_reset");

       if (error) {
         dev_err(dev, "unable to request ir reset gpio %d\n",UEI_IR_RESET);
       }

  gpio_direction_output(UEI_IR_RESET,1);
