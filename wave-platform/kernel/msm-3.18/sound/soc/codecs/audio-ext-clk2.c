...
static struct clk_lookup audio_ref_clock_1[] = {
...
static int audio_get_pinctrl(struct platform_device *pdev, enum clk_mux mux)
{
	...
	pnctrl_info->sleep = pinctrl_lookup_state(pinctrl, "sleep_1");
	if (IS_ERR(pnctrl_info->sleep)) {
		dev_err(&pdev->dev, "%s: could not get sleep pinstate\n",
			__func__);
		goto err;
	}
	pnctrl_info->active = pinctrl_lookup_state(pinctrl, "active_1");
...
static int audio_ref_clk_probe(struct platform_device *pdev)
{
	...
	if (ret)
		dev_dbg(&pdev->dev, "%s: qcom,lpass-clock_1 is undefined\n",
				__func__);
	...
	if (!ret && (mclk_freq == 12288000 || audio_lpass_mclk.lpass_clock)) {
		...
		ret = of_msm_clock_register(pdev->dev.of_node, audio_ref_clock_1,
			      ARRAY_SIZE(audio_ref_clock_1));
		if (ret)
...
static const struct of_device_id audio_ref_clk_match[] = {
	{.compatible = "qcom,audio-ref-clk_1"},
	{}
};
