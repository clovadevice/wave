...
CPPFLAGS_append = " -DUSE_BT_OBEX -DUSE_LIBHW_AOSP"
...
do_install_append() {
        ...
        if [ -f ${S}conf/bt_qci.conf ]; then
           install -m 0660 ${S}conf/bt_qci.conf ${D}${sysconfdir}/bluetooth/
        fi
        if [ -f ${S}conf/bt_qci_spp.conf ]; then
           install -m 0660 ${S}conf/bt_qci_spp.conf ${D}${sysconfdir}/bluetooth/
        fi
}
